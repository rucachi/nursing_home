...
OUTPUT_CSV = r'd:\anti_gravity\nursing home\data\nursing_facilities_analysis_data.csv'
...
def calculate_vam_score(row):
    ...
    if pd.notna(capacity) and pd.notna(staff) and capacity > 0:
...
def main():
    ...
    query = '''
        SELECT f.id, f.facility_type, f.sido, f.sigungu, f.name, f.capacity, f.staff_count, 
               f.address, f.lat, f.lon, 
    ...
    print(df[['name', 'scale_group', 'distance_group', 'age_group', 'vam_score']].head())
